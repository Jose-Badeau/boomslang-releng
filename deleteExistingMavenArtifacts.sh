#!/bin/bash
# Sample Usage: deleteExistingMavenArtifacts.sh username apikey owner repo package version
###############################################
...
function deploy_updatesite() {
  ...
  curl -X POST -u ${BINTRAY_USER}:${BINTRAY_API_KEY} "https://api.bintray.com/packages/${BINTRAY_OWNER}/${BINTRAY_REPO}/${PCK_NAME}/versions" -d '{"name": "1.0.0", "desc": "Release 1.0.0"}' -H "content-type:application/json" 
  echo ""
}
